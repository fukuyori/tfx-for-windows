...
Status: **Done.** Search is **always** a recursive walk of the current folder's subtree; there is no recursive-mode toggle (removed per user feedback). Typing in the search box does not auto-trigger; pressing **Enter** starts the walk, **Esc** clears the box and reloads the real folder listing, and any folder navigation cancels the in-flight search. Matches stream into the active pane's `ObservableCollection` in batches of 50. The status line is rendered by `UpdateStatus` from the search state while results are shown — `Searching "q"  12.3s  scanned N  matched M`, refreshed every 250 ms by a UI timer from an `Interlocked` scanned counter, then `Search complete "q"  ...` until the user leaves search mode (Esc in the box or the list, empty Enter, navigation, reload). Up / Down from the search box step through the results. The diff auto-refresh skips a pane holding search results. File operations on results keep the pane in search mode: delete / move-out prune the affected rows (`PruneSearchResults`), rename rebuilds the row and any rows under a renamed folder (`ReplaceSearchResultAfterRename`; the inline editor shows only the file name); operations that create something in the searched folder itself (paste / drop into it, new item, zip, extract) leave search mode via `Reload`. Results show the relative path in the Name cell while preserving `FullPath` so Open / Reveal still work. Inside zip archives the recursive walk is a no-op (the archive listing is already flat). Move this section under §1 with a version tag at the next release bump.
...
Status: **Done in 0.6.5; lifecycle refined in 0.6.6.** Collapsible terminal pane docked at the window bottom, rendered by **xterm.js** (the VS Code terminal engine) hosted in WebView2, with shell I/O through tfx's own ConPTY wrapper (`src/Services/ConPty.cs`). A first attempt used a self-written VT parser (`TerminalView.cs` / `TerminalPalette.cs`); it was replaced by xterm.js to get correct CJK width, 24-bit + 256 color, scrollback, and full-screen TUI support (vim/less). xterm assets are embedded in the executable (single-file friendly), extracted to `%LOCALAPPDATA%\tfx\terminal` at startup, and served from a local virtual host (no CDN). Toggle via the toolbar button / `Ctrl+J` / context-menu (default is `Ctrl+J` rather than `Ctrl+\`` because the backtick key is IME-bound at the 半角/全角 position on JP keyboards; remappable via `[shortcuts] toggleTerminal`); resize by splitter; close with header `×` or `exit`. Shell, font, size, and the full ANSI palette come from `config.toml [terminal]` (`shell` / `font` / `fontSize` / colors); omitting `background` keeps the surface transparent. Started in the active pane's folder (startup cwd only). Visibility / height persist in `settings.json`. **0.6.6** lifecycle: closing the pane disposes the shell but keeps the xterm page; reopening resets the screen and spawns a fresh shell; clicking in the pane focuses it; the shell is torn down on app exit so no orphaned pseudo console lingers. The external launcher (§0.5.2) stays the default hand-off path. Deferred: live folder-follow `cd`, run-command-on-selected-files. Move this section under §1 at the next consolidation pass.
