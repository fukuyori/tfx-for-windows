...
タイトルバーの歯車ボタンで設定メニュー（「**設定ファイルを編集...**」「**エディター設定...**」「**ターミナル設定...**」）が開きます。同じ項目はファイルペインの右クリックメニューにもあります。「**設定ファイルを編集...**」または `editConfig` ショートカット（既定 `Ctrl+,`）で `config.toml` がエディターで開きます。エディターは次の順で選ばれます。

1. 「**エディター設定...**」で指定したコマンド（`settings.json` に保存。引数の `{path}` は設定ファイルのパスに展開、環境変数も展開）
2. OS の `.toml` 関連付け
3. メモ帳

tfx は `config.toml` を監視しており、保存すると即座に再適用されます — 再起動は不要です。ショートカット（ツールバーのツールチップ含む）、`[colors]` / `[opacity]`、`[font]`、`[openWith]`、`[[commands]]`、内蔵ターミナルの配色・フォントはすぐに反映されます。設定にエラーがある場合（起動時・リロード時とも）はエラー一覧をダイアログとステータスバーに表示します。各エラーには `config.toml` の行番号が付きます（例: `line 12: Invalid color for ...`）。アプリは動作を続け、不正な値は直前の設定または既定値にフォールバックします。

即時反映されないもの:

- `[startup]` — 起動時にのみ読み込まれ、次回起動時に反映されます。
- `[terminal] shell` と背景透過（`[terminal] background` の追加/削除）— ターミナルペインを開き直したときに反映されます。
