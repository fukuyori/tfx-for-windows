...
- フォルダーツリー（ツールバーで表示/非表示の切替・全折りたたみボタン。シングルクリックで選択、ダブルクリックでフォルダーを開く/閉じる）、ピン留めフォルダー、編集可能なアドレスバー、クリック可能な breadcrumb。
- **Details** と **Icons** の 2 表示モード。ファイル種別ごとのテーマ色に追従する単色アイコン。
- New File / New Folder、インライン rename、Windows の修飾キーに沿ったドラッグ & ドロップ（Explorer 風の半透明ドラッグプレビュー付き）、`.lnk` ショートカット作成。
