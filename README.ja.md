...
- フォルダーツリー（ツールバーで表示/非表示の切替・全折りたたみボタン。シングルクリックでファイル一覧に反映、ダブルクリックで展開/折りたたみ）、ピン留めフォルダー、編集可能なアドレスバー、クリック可能な breadcrumb。ファイル一覧からツリーのフォルダーへドロップ可能（同一ドライブは移動・別ドライブはコピー、修飾キーと右ドラッグメニュー対応。ドラッグ中フォルダーの自身/サブフォルダーへは不可、ホバーで自動展開）。
- サイドバーの「ディスク」セクション: 各ドライブを使用量バー付きで表示（ツールチップに空き/合計）。クリックでそのドライブをアクティブなファイル一覧に表示。
- **Details** と **Icons** の 2 表示モード。ファイル種別ごとのテーマ色に追従する単色アイコン。
- New File（`.txt`）/ New Folder は Explorer 風のインライン命名（名前を入力して Enter、または一覧の空き場所クリックで確定）、インライン rename、Windows の修飾キーに沿ったドラッグ & ドロップ（Explorer 風の半透明ドラッグプレビュー付き）、`.lnk` ショートカット作成。Explorer のごみ箱ウィンドウからドロップ（または切り取り→貼り付け）した項目は、修飾キーに関わらず移動として扱い、元の名前で復元します。
...
| `Ctrl + Shift + I` | 現在のプレビューで画像（ローカル / 外部）を読み込む（表示可能なとき） |
...
タイトルバーの歯車ボタンで設定メニュー（「**設定ファイルを編集...**」「**エディター設定...**」「**ターミナル設定...**」）が開きます。「設定ファイルを編集...」（`Ctrl+,` やファイルペインの右クリックメニューからも可）は `config.toml` をエディターで開きます。使用するエディターは「**エディター設定...**」で指定でき、未指定なら OS の `.toml` 関連付け、それも無ければメモ帳で開きます。tfx はファイルを監視しており、保存すると即座に再適用されます — ショートカット・カラー・フォント・コマンド・ターミナル配色は再起動なしで反映されます。例外: `[startup]` は次回起動時、内蔵ターミナルの `shell` と背景透過はターミナルペインを開き直したときに反映されます。
