...
- **Search status no longer alternates with the folder summary.** The search summary used to be written directly to the status bar while `UpdateStatus` (selection changes, refreshes) kept overwriting it with `path  N items`, so the two flickered against each other. `UpdateStatus` now renders the search summary itself whenever the active pane holds search results, from start until the user leaves search mode (Esc, empty Enter, navigation, reload) — including after completion, where it previously reverted on the first selection change.
- **Esc cancels the search from the results list too.** Esc was handled only inside the search box; once focus moved into the list, the only way out of search mode was navigating away. Esc outside any text box (rename and address-bar editing keep their own Esc) now clears the box, stops the walk, and restores the real listing in the pane that held the results.
- **Up / Down from the search box step through the results.** The arrow-key forwarding to the listing applied only while the box was empty; after Enter the query stays in the box, so there was no keyboard path into the results. Forwarding now also applies while search results are shown.
- **File operations inside search results keep you in the results.** Deleting a result, moving results out (cut + paste, drag to another folder), or renaming one used to reload the pane and drop back to the real folder listing. The pane now stays in search mode: deleted and moved-away rows are removed (only the affected paths are probed, so large result sets aren't re-checked), and a renamed row — plus every result under a renamed folder — is rebuilt with its new path and stays selected. Inline rename of a result now edits just the file name; the cell used to hand the whole relative path to the editor, which failed validation for anything below the top level. Operations that create something in the searched folder itself (paste or drop into it, new file / folder, zip, extract) still leave search mode to show the real listing.
- **`-v` / `--version` command-line option.** Prints `tfx X.Y.Z` (the same version as the title bar and status bar) and exits with code 0; without a console it shows a message box, like `--help`. Listed in the `--help` text and the README option tables. The version lookup moved into a shared `AppVersion` helper used by both the CLI and the main window.
- **`--help` / `--version` output can be redirected and piped.** When the parent redirected standard output (`tfx --version > file`, `tfx --version | Out-String`, `cmd /c tfx --version` in a script), the text is written to that inherited handle before any console attach; previously `AttachConsole` repointed the standard handles at the console screen buffer, so the redirection target stayed empty. Without redirection the console path is unchanged. Note that PowerShell's plain `$v = tfx --version` still captures nothing: PowerShell does not open a pipe for a GUI-subsystem executable unless another command follows in the pipeline, so use `tfx --version | Out-String` (or `cmd /c`) there.
- **`build-zip.ps1` gained `-Sign`** (with `-CertSubject` / `-SignToolPath` / `-TimestampUrl`, same as `build-installer.ps1`). It signs the published `Tfx.exe` in place and verifies it before zipping; a file already validly signed by the same certificate is left untouched, so running the installer script with `-Sign` first does not prompt for the token PIN again. Previously the portable ZIP was signed only if the installer script had happened to run first.
- **Auto-refresh no longer wipes search results.** A `FileSystemWatcher` event (or the periodic poll on network shares) during or after a search ran the diff refresh against the real folder listing; since result rows are named by relative path, they matched nothing and were all removed. The diff refresh now skips a pane that is showing search results.
